...
<CardBody>
  <div className="d-flex">
    ...
    {props.parent === false ? (
      <button className="btn btn-sm btn-primary" disabled={isLoading} onClick={() => { toast.success('Reminder is Sent') }}>Send Reminder</button>
   
  ) :
  
  (
(  !props.teacher &&    <button className="btn btn-sm btn-primary" disabled={isLoading} onClick={MarkAsRead}>Mark As Read</button>
)    )}

<button className="btn btn-sm btn-primary" style={{marginLeft:'10px'}}  onClick={() => { navigate(`/teacher/student/contact-book/${props.id}`) }}>Read The Form</button>
