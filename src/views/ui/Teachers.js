...
import { useAllTeachersQuery, useDeleteTeacherMutation } from '../../slices/teacherApiSlice';
...
const {data, isLoading, error, refetch} = useAllTeachersQuery()
const [deleteGuardian, {isLoading:isDeleteLoaing}] = useDeleteTeacherMutation()
    ...
    <tbody>
      ...
      {data.map((guardian) => (
        <tr key={guardian.user.id} className="border-top">
          ...
          <td><Button onClick={()=> navigate(`/teachers/${guardian.user.id}`)} className="btn" color="info">
     More
    </Button></td>
    <td>
